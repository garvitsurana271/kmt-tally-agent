...
// ── Stock summary sync (closing qty per item → tally_stock_summary) ──
let lastStockSummarySync = 0;
async function syncStockSummary() {
  if (Date.now() - lastStockSummarySync < MASTER_TTL) return;
  try {
    log("Fetching stock summary from Tally…");
    const items = await tally.getStockSummary();
    log(`Got ${items.length} stock items with quantities from Tally`);
    if (items.length > 0) {
      const res = await db.upsertStockSummary(items);
      log(`Stock summary synced: ${items.length} items (HTTP ${res?.status})`);
    }
    lastStockSummarySync = Date.now();
  } catch (e) { err("Stock summary sync failed:", e.message); }
}
